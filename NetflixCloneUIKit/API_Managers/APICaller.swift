...
class APICaller {
    ...
    // getting Tv movies
    func getTvMovies(completion: @escaping (Result<[Tv], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/3/tv/day?api_key=\(Constants.API_KEY)") else {
            return
        }
    ...
    func getPopularMovies(completion: @escaping (Result<[Movie], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/3/popular?api_key=\(Constants.API_KEY)&language=en-US&page=1") else {
            return
        }
